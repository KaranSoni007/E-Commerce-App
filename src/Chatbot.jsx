...
const ProductCardInChat = memo(({ product, closeChat }) => {
  ...
  return (
    ...
    >
      ...
      <div className="p-2">
        <p className="text-xs font-semibold text-gray-800 line-clamp-2 h-8">{product.title}</p>
        <p className="text-sm font-bold text-indigo-600 mt-1">{formatPrice(product.OriginalPrice)}</p>
      </div>
    </div>
  );
});
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-md overflow-hidden mt-2 w-full max-w-[95%] text-left">
      ...
      <div className="p-4 space-y-3">
        <h4 className="font-bold text-gray-900 text-sm leading-snug">{product.title}</h4>
        
        <div className="flex items-center gap-2">
            <span className="text-lg font-bold text-indigo-600">{formatPrice(product.OriginalPrice)}</span>
            {product.MRP && <span className="text-xs text-gray-400 line-through">{formatPrice(product.MRP)}</span>}
        </div>
             ...
             <div className="text-xs text-gray-600 bg-gray-50 p-2 rounded border border-gray-100">
                <div className="grid grid-cols-1 gap-1">
                    {Object.entries(product.specifications).slice(0, 4).map(([key, value]) => (
                        <div key={key} className="flex justify-between border-b border-gray-200 last:border-0 pb-1 last:pb-0">
...
const Chatbot = () => {
  ...
  const [messages, setMessages] = useState([
    {
      id: 1,
      type: "bot",
      text: "Hi there! 👋 I'm IntelliBot. I can help you find products, track orders, or answer support questions.",
    },
  ]);
  ...
  const generateResponse = async (text) => {
    ...
    // We check if the input matches any product title
    ...
    if (lowerText.includes("order") || lowerText.includes("track") || lowerText.includes("status")) {
      if (user) {
        return { text: "You can view and track your recent orders in the 'My Profile' section. Would you like me to take you there?" };
      }
      return { text: "Please log in to track your orders. I can help you find the login page!" };
    }
    if (lowerText.includes("return") || lowerText.includes("refund")) {
      return { text: "We have a hassle-free 30-day return policy. If you're not satisfied, you can initiate a return from your Order History." };
    }
    ...
    if (lowerText.includes("offer") || lowerText.includes("discount")) {
      return { text: "We currently have a 20% OFF sale! Use code 'INTERN20' at checkout." };
    }
    if (lowerText.includes("phone") || lowerText.includes("mobile")) {
        const phones = AllProducts.filter((p) => p.category === "Smartphones").slice(0, 3);
        return {
    ...
    return { text: "I'm still learning! You can ask me about products, shipping, returns, or check our Support page for more details." };
  };
  ...
  return (
    <>
      ...
      <motion.button
        ...
        className="fixed bottom-6 right-6 z-[110] w-14 h-14 bg-indigo-600 text-white rounded-full shadow-lg flex items-center justify-center hover:bg-indigo-700 transition-colors focus:outline-none focus:ring-4 focus:ring-indigo-300"
      >
        {isOpen ? (
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-24 right-4 md:right-6 z-[110] w-[90vw] md:w-96 h-[500px] max-h-[80vh] bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col overflow-hidden font-sans"
          >
            {/* Header */}
            <div className="bg-indigo-600 p-4 flex items-center gap-3 shrink-0">
              ...
              <div>
                <h3 className="text-white font-bold text-lg leading-tight">IntelliBot</h3>
              ...
              <button
                ...
                className="ml-auto p-2 text-indigo-200 hover:text-white hover:bg-indigo-500 rounded-full transition-colors"
                title="Clear Chat"
              >
            ...
            {messages.length < 4 && !isTyping && (
                <div className="px-4 py-2 bg-gray-50 overflow-x-auto flex gap-2 no-scrollbar shrink-0">
            ...
            <div className="p-3 bg-white border-t border-gray-100 shrink-0">
              <form onSubmit={handleSendMessage} className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-4 py-2.5 bg-gray-100 border-none rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all outline-none"
                />
                <button
                  type="submit"
