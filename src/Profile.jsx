...
function Profile() {
  ...
  const handleNotificationToggle = (type) => {
    ...
    localStorage.setItem(
      "notificationPrefs_" + userData.email,
  ...
  const openEditAddressForm = (addressItem) => {
    ...
    if (addressItem.fullName || addressItem.phone || addressItem.street) {
      parsedData = {
        fullName: addressItem.fullName || userData.name || "",
  ...
  const saveAddress = () => {
    ...
    if (editingAddress) {
      const index = allAddresses.findIndex(
        (a) => a.userEmail === userData.email && a.text === editingAddress.text,
      );
      if (index !== -1) {
  ...
  const handleSaveProfile = () => {
    ...
    const existingUsers = JSON.parse(localStorage.getItem("mockUsers")) || [];
    const updatedUsers = existingUsers.map((u) =>
      u.email === userData.email ? { ...u, name: editName } : u,
    );
    localStorage.setItem("mockUsers", JSON.stringify(updatedUsers));
  ...
  const handleDeleteAccount = () => {
    ...
    if (isConfirmed) {
      ...
      localStorage.removeItem("userName");
      localStorage.removeItem("userId");
      localStorage.removeItem("userEmail"); // 🔹 Added missing cleanup
      window.dispatchEvent(new Event("userUpdated")); // 🔹 Update Navbar immediately
                      ...
                      {orders.map((order, index) => (
                        ...
                        >
                          <div className="flex justify-between items-center mb-4">
                            ...
                            <span className="bg-emerald-100 text-emerald-700 px-3 py-1 rounded-md text-xs font-bold">
                              Confirmed ✅
                            </span>
                          </div>
                      ...
                      <div>
                        ...
                        <div className="flex gap-2">
                          <input
                            ...
                            className="flex-1 p-2.5 border border-gray-300 rounded-lg text-sm outline-none focus:border-indigo-600 focus:shadow-[0_0_0_3px_rgba(79,70,229,0.1)] transition-all"
                          />
                          <button
                    ...
                    <div className="mt-4 space-y-4 animate-fadeIn">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full p-2.5 border border-gray-300 rounded-lg text-sm outline-none focus:border-indigo-600 focus:shadow-[0_0_0_3px_rgba(79,70,229,0.1)] transition-all"
                        />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-gray-600 mb-1">
